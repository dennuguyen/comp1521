...
{
    int n_bcd = x->n_bcd > y->n_bcd ? x->n_bcd : y->n_bcd; // get larger n_bcd
    char *string = malloc(sizeof(char) * (n_bcd + 1));
    int carry = 0;
    for (int i = 0; i < n_bcd; i++)
    {
        int x_num = i < x->n_bcd ? x->bcd[i] : 0;
        int y_num = i < y->n_bcd ? y->bcd[i] : 0;

        string[i] = (x_num + y_num + carry) % 10 + '0';
        carry = (x_num + y_num + carry) / 10;
    }

    // deal with leftover carry
    if (carry != 0)
    {
        string = realloc(string, sizeof(char) * (++n_bcd + 1));
        string[n_bcd - 1] = carry + '0';
    }

    string[n_bcd] = '\0';

    // reverse the string
    for (char *start = string, *end = string + n_bcd - 1; start < end;)
    {
        char tmp = *start;
        *start++ = *end;
        *end-- = tmp;
...
//     for (int i = n_bcd; i >= 0; i--)
//     {
//         string[i] = x->bcd[i] | y->bcd[i];
//     }
//     // get digit
...
{
    // multiply by 0
    if ((x->n_bcd == 1 && *x->bcd == 0) || (y->n_bcd == 1 && *y->bcd == 0))
        return bcd_from_string("0");

    // allocate memory for z and z->bcd
    big_bcd_t *z = malloc(sizeof *z);
    z->bcd = malloc(x->n_bcd * sizeof z->bcd[0]);
    *z->bcd = *x->bcd;
    z->n_bcd = x->n_bcd;

    // multiply by 1
    if (x->n_bcd == 1 && *x->bcd == 1)
    {
        *z->bcd = *y->bcd;
        z->n_bcd = y->n_bcd;
        return z;
    ...
    if (y->n_bcd == 1 && *y->bcd == 1)
        return z;

    big_bcd_t *buffer = bcd_from_string("1");

    // in other words add x, y times
    while (!(y->n_bcd == 1 && *y->bcd == 1))
    {
        z = bcd_add(x, z);
        y = bcd_subtract(y, buffer);
    }
